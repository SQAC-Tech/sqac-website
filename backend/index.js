...
if (process.env.MONGO_URI) {
  ...
  console.warn('MONGO_URI is undefined. Mongo-backed routes will fail until it is configured.');
}
...
app.get('/api/cron/sync', async (req, res) => {
  ...
  try {
    const workbookPath = resolveExistingFile([
      path.join(__dirname, 'data.xlsx'),
      path.join(__dirname, 'Member Information Form For SQAC Website (Responses).xlsx'),
      path.join(__dirname, '..', 'Frontend', 'api', 'data.xlsx'),
      path.join(__dirname, '..', 'Frontend', 'api', 'Member Information Form For SQAC Website (Responses).xlsx'),
    ]);

    if (!workbookPath) {
      throw new Error('No workbook file was found for sync.');
    }
